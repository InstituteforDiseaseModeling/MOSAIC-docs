...
{\emph{
Website under development. Last compiled on 2026-06-01 at 05:46 PM PDT.
}}
...
\caption{A map of Sub-Saharan Africa with countries that have experienced a cholera outbreak in the past 5 and 10 years highlighted in green. The 40 countries included in the MOSAIC modeling framework are indicated in blue.}\label{fig:map}
\end{figure}
...
\caption{Example of a grid of 30 uniformly distributed points within Mozambique (A). The scatterplot shows weekly summed precipitation values at those 30 grid points and cholera cases plotted on the same scale of the Z-Score which shows the variance around the mean in terms of the standard deviation. Fitted Fourier series functions are shown as blue (fit precipitation data) and red (fit to cholera case data) lines.}\label{fig:seasonal-example}
\end{figure}
...
To reflect this evidence base we compiled a meta-analysis of 13 published infectious-dose estimates spanning four types of source: direct human-volunteer challenge studies, Beta-Poisson QMRA fits, expert-review summaries, and conventional modelling values. The volunteer challenge studies (Hornick et al.~1971, \href{https://doi.org/10.1093/infdis/129.1.45}{Cash et al.~1974}, Levine et al.~1981, \href{https://doi.org/10.1016/S0140-6736(88)90120-1}{Levine et al.~1988}, Tacket et al.~1999) provide the strongest primary evidence; QMRA syntheses (\href{https://www.wiley.com/en-us/Quantitative+Microbial+Risk+Assessment-p-9780471183976}{Haas, Rose \& Gerba 1999}) and expert reviews (\href{https://doi.org/10.1128/cmr.8.1.48}{Kaper et al.~1995}, \href{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3842031/}{Nelson et al.~2009}) provide supporting summaries; and conventional modelling values (\href{https://doi.org/10.1186/1471-2334-1-1}{Codeço 2001}, \href{https://journals.plos.org/plosmedicine/article?id=10.1371/journal.pmed.0030007}{Hartley et al.~2006}) capture the values adopted in the cholera-transmission literature. Each source is weighted by its evidentiary quality and relevance to the endemic, buffered-exposure context: direct volunteer challenge studies receive a weight of 1.0, while expert reviews and QMRA syntheses receive weights between 0 and 0.5. A weighted lognormal is then fit by maximum-likelihood on the log10 scale:
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0773}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.3866}}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1546}}@{}}
\toprule\noalign{}
...
\caption{The estimated vaccination coverage across all countries with reported vaccination data on the WHO ICG dashboard.}\label{fig:vaccination-countries}
\end{figure}
...
\caption{\label{tab:immunity-sources}Sources for the duration of immunity from natural infection.}
\centering
...
We estimated the mean immune decay to be \(\bar\varepsilon \approx 3.9 \times 10^{-4}\) day\(^{-1}\), equivalent to a mean immune duration of approximately 7 years as shown in Figure \ref{fig:immune-decay}A. This is slightly longer than previous modeling work estimating the duration of immunity to be approximately 5 years (\href{https://www.nature.com/articles/nature07084}{King et al.~2008}). Uncertainty around \(\varepsilon\) in the model is represented by a lognormal prior parameterised on the natural scale by the mean and standard deviation (matching the convention used in \texttt{priors\_default.json}), as shown in Figure \ref{fig:immune-decay}B:
...
The \(\rho_{\text{deaths}}\) prior is fit by quantile-matching a random-effects meta-analysis (DerSimonian-Laird, logit scale) of three SSA studies that directly measure cholera-death surveillance capture: \href{https://doi.org/10.3201/eid2313.170529}{Routh et al.~2017} (Tanzania, 0.48), \href{https://doi.org/10.4269/ajtmh.2009.09-0400}{Shikanga et al.~2009} (Kenya, 0.34), and \href{https://doi.org/10.1371/journal.pntd.0002545}{Bwire et al.~2013} (Uganda, 0.50). The pooled mean is 0.42 with a 95\% prediction interval of {[}0.16, 0.73{]} (\(\tau^2 = 0.046\), \(I^2 = 32\%\)); see Figure \ref{fig:rho-deaths-prior}. The conceptual framework separating facility CFR from community deaths comes from \href{https://doi.org/10.1016/S1473-3099(24)00237-8}{Finger et al.~2024} (\emph{Lancet Infect Dis}). Reported deaths are then modelled analogously to reported cases using the dynamic infection-fatality ratio described in the \hyperref[case-fatality-rate]{Case fatality rate} subsection.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{figures/rho_deaths_prior} 

}

\caption{Prior for the cholera death-detection probability $\rho_{\text{deaths}}$. (A) Recommended Beta(6.30, 8.52) (blue, solid) fit by quantile-matching the 5th, 50th, and 95th percentiles of the 95\% prediction interval from a random-effects meta-analysis of three SSA direct-capture studies; informative alternative Beta(36.95, 51.02) (plum, dashed) fit to the 95\% CI of the pooled mean. (B) Per-study estimates and the pooled random-effects estimate (logit-scale DerSimonian-Laird, k = 3, $\tau^2$ = 0.046, $I^2$ = 32\%).}\label{fig:rho-deaths-prior}
\end{figure}
...
\hline
AFRO Region & 1135678 & 21942 & 0.019 & 0.019 & 0.020 & 0.008 & 1.909\\
\hline
Angola & 3942 & 108 & 0.027 & 0.023 & 0.033 & 0.010 & 1.931\\
\hline
Burundi & 5621 & 40 & 0.007 & 0.005 & 0.010 & 0.007 & 1.918\\
\hline
Benin & 3617 & 56 & 0.015 & 0.012 & 0.020 & 0.008 & 1.906\\
\hline
Burkina Faso & 7 & 0 & 0.019 & 0.019 & 0.020 & 0.008 & 1.909\\
\hline
Cote d'Ivoire & 446 & 18 & 0.040 & 0.024 & 0.063 & 0.013 & 1.863\\
\hline
Cameroon & 29897 & 925 & 0.031 & 0.029 & 0.033 & 0.010 & 1.931\\
\hline
Democratic Republic of Congo & 310792 & 5992 & 0.019 & 0.019 & 0.020 & 0.008 & 1.910\\
\hline
Congo & 435 & 36 & 0.083 & 0.059 & 0.113 & 0.019 & 1.905\\
\hline
Ethiopia & 46877 & 660 & 0.014 & 0.013 & 0.015 & 0.008 & 1.893\\
\hline
Ghana & 29814 & 251 & 0.008 & 0.007 & 0.010 & 0.007 & 1.925\\
\hline
Guinea & 1 & 0 & 0.019 & 0.019 & 0.020 & 0.008 & 1.909\\
\hline
Guinea-Bissau & 11 & 2 & 0.019 & 0.019 & 0.020 & 0.008 & 1.909\\
\hline
Kenya & 47343 & 678 & 0.014 & 0.013 & 0.015 & 0.008 & 1.920\\
\hline
Liberia & 580 & 0 & 0.000 & 0.000 & 0.006 & 0.006 & 1.938\\
\hline
Mali & 12 & 4 & 0.019 & 0.019 & 0.020 & 0.008 & 1.909\\
\hline
Mozambique & 84144 & 351 & 0.004 & 0.004 & 0.005 & 0.006 & 1.894\\
\hline
Malawi & 63879 & 1859 & 0.029 & 0.028 & 0.030 & 0.010 & 1.879\\
\hline
Namibia & 698 & 13 & 0.019 & 0.010 & 0.032 & 0.010 & 1.912\\
\hline
Niger & 11639 & 334 & 0.029 & 0.026 & 0.032 & 0.010 & 1.901\\
\hline
Nigeria & 241522 & 6521 & 0.027 & 0.026 & 0.028 & 0.009 & 1.894\\
\hline
Rwanda & 478 & 0 & 0.000 & 0.000 & 0.008 & 0.006 & 1.919\\
\hline
Sudan & 362 & 11 & 0.030 & 0.015 & 0.054 & 0.012 & 1.855\\
\hline
Somalia & 134839 & 1849 & 0.014 & 0.013 & 0.014 & 0.008 & 1.906\\
\hline
South Sudan & 35871 & 715 & 0.020 & 0.019 & 0.021 & 0.009 & 1.914\\
\hline
Eswatini & 2 & 0 & 0.019 & 0.019 & 0.020 & 0.008 & 1.909\\
\hline
...
\hline
Zambia & 11373 & 271 & 0.024 & 0.021 & 0.027 & 0.009 & 1.892\\
\hline
...
\begin{table}
\centering
\caption{\label{tab:demographics}Demographics for AFRO countries in 2023. Data include: total population as of January 1, 2023, daily birth rate, and daily death rate. Values are calculated from crude birth and death rates from UN World Population Prospects 2024.}
\centering
...
Following the ``upper-bound'' convention for \(R_0\), the pathogen-decay rate is fixed at its \textbf{slowest} plausible value, taken as the modal upper bound of the v1.0 decay prior \(\text{days}_{\text{long}} = \text{days}_{\text{short}} + \text{days}_{\text{spread}}\):
...
corresponding to a survival time of approximately 196 days in water (16-day mode of \(\text{days}_{\text{short}}\) plus 180-day mode of \(\text{days}_{\text{spread}}\); see Equation \eqref{eq:decay-priors}).
...
\item
  \textbf{\(R_{0,j}^{\text{env}}\)} grows when WASH access is poor (\(\theta_j\!\to\!0\)) and pathogen survival is long (\(\widehat{\delta}_j \approx 1/196\)).
...
\caption{\label{tab:mosaic-table}List of MOSAIC Countries with Cholera News}
\centering
...
\hline
\$\textbackslash{}nu\_\{jt\}\$ & Total OCV vaccination rate in destination \$j\$ at time \$t\$. In MOSAIC v1.0 this is split into separate first-dose (\$\textbackslash{}nu\_\{1,jt\}\$) and second-dose (\$\textbackslash{}nu\_\{2,jt\}\$) rates --- see the supplementary parameter table below for details. &  & \\
\hline
...
\hline
\$\textbackslash{}mu\_\{j,t\}\$ & Dynamic infection-fatality ratio (per-day mortality hazard) among symptomatic individuals. Decomposed into \$\textbackslash{}mu\_\{j,0\}, \textbackslash{}mu\_\{j,1\}, \textbackslash{}mu\_\{j,\textbackslash{}text\{epi\}\}\$ (see the *Case fatality rate* subsection). &  & \\
\hline
...
\hline
\$\textbackslash{}delta\_\{\textbackslash{}text\{min\}\}\$ & Minimum decay rate (longest *V. cholerae* survival), reached at \$\textbackslash{}psi\_\{jt\}=1\$; equals \$1/\textbackslash{}text\{days\}\_\{\textbackslash{}text\{long\}\}\$. & \$\textbackslash{}approx 0.0051 \textbackslash{} (196 \textbackslash{} \textbackslash{}text\{days\})\$ & \\
\hline
\$\textbackslash{}delta\_\{\textbackslash{}text\{max\}\}\$ & Maximum decay rate (shortest *V. cholerae* survival), reached at \$\textbackslash{}psi\_\{jt\}=0\$; equals \$1/\textbackslash{}text\{days\}\_\{\textbackslash{}text\{short\}\}\$. & \$\textbackslash{}approx 0.063 \textbackslash{} (16 \textbackslash{} \textbackslash{}text\{days\})\$ & \\
\hline
...
\hline
\$\textbackslash{}kappa\$ & Number of *V. cholerae* cells required for 50\% per-contact infection probability. In MOSAIC v1.0 expressed in absolute cells (matching the units of \$W\$); see [Infectious dose (\$\textbackslash{}kappa\$)](\#infectious-dose-kappa). & \$\textbackslash{}text\{Lognormal\}(11.77, 1.82)\$ & Meta-analysis (see *Infectious dose* subsection)\\
\hline
\end{tabular}
...
\hline
\$\textbackslash{}psi\textasciicircum{}\{\textbackslash{}ast\}\_\{jt\}\$ & Calibrated (EWMA-smoothed, logit-affine, time-shifted) environmental suitability used in the FOI and decay rate (see the *Calibration of suitability to surveillance* subsection). & Computed (Eq. \textbackslash{}@ref(eq:psi-star)). & Computed (per-country logit calibration).\\
\hline
...
\hline
\$\textbackslash{}rho\_\{\textbackslash{}text\{deaths\}\}\$ & Probability that a true cholera death is captured by surveillance. & \$\textbackslash{}text\{Beta\}(6.30, 8.52)\$ & SSA random-effects meta-analysis: Routh 2017, Shikanga 2009, Bwire 2013.\\
\hline
...
\hline
\$- \textbackslash{}mu\_\{j,t\} I\_\{1,jt\}\$ & Cholera-attributable mortality among symptomatic individuals (dynamic IFR; see the *Case fatality rate* subsection). & \$\textbackslash{}text\{Binom\}\textbackslash{}big( I\_\{1,jt\},\textbackslash{}; 1 - \textbackslash{}exp(-\textbackslash{}mu\_\{j,t\}) \textbackslash{}big)\$\\
\hline
...
\hline
\$+ \textbackslash{}zeta\_1 I\_\{1,jt\}\$ & Cells shed into the environment by symptomatic individuals. & \$(1-\textbackslash{}theta\_j)\textbackslash{},\textbackslash{}text\{Pois\}\textbackslash{}big( \textbackslash{}zeta\_1 I\_\{1,jt\} \textbackslash{}big)\$\\
\hline
\$+ \textbackslash{}zeta\_2 I\_\{2,jt\}\$ & Cells shed into the environment by asymptomatic individuals. & \$(1-\textbackslash{}theta\_j)\textbackslash{},\textbackslash{}text\{Pois\}\textbackslash{}big( \textbackslash{}zeta\_2 I\_\{2,jt\} \textbackslash{}big)\$\\
\hline
...
In the simplified SVEIWRS structure introduced in laser-cholera 0.12, only the \emph{effective} fraction of each delivered dose enters \(V_1\) or \(V_2\); ineffective doses are returned (notionally) to the source compartment without changing anyone's compartment membership. Total dose counts on day \(t\) are tracked separately as \(\text{doses}^{(1)}_{j,t}\) and \(\text{doses}^{(2)}_{j,t}\) for comparison with reported OCV-campaign data; these counters are stored as patch-level vectors of length \(T_{\text{total}}\) and are not used in the model dynamics.
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.3874}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.6126}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Parameter
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\end{equation}
The threshold of \(\Delta_i \le 6\) is widely used in model selection and corresponds approximately to a likelihood ratio of \(p_i = \exp(-\Delta_i/2) \approx 0.05\), which in nested-model comparisons aligns loosely with a frequentist \(p\)-value of 0.05 (Burnham \& Anderson \href{https://doi.org/10.1007/b97636}{2002} and \href{https://journals.sagepub.com/doi/abs/10.1177/0049124104268644}{2004}). Since every MOSAIC simulation has the same number of estimated parameters \(k\), \(\Delta_i\) reduces to a likelihood-ratio statistic and the cut-off acts as a likelihood-ratio filter on the BFRS draws rather than as a formal model-selection criterion across nested models. This cut-off removes models with essentially no empirical support, while preserving relative likelihood ratios among the retained models. Alternative posterior diagnostics such as the Pareto-\(k\) shape statistic of PSIS (\href{https://doi.org/10.48550/arXiv.1507.02646}{Vehtari et al.~2024}) can be used to flag cases where the truncated importance-weight distribution has unbounded variance.
...
We quantify consensus among the retained subset of best models \(\mathcal B = \{\,i : \Delta_i \le 6\}\) by the normalized \href{https://en.wikipedia.org/wiki/Entropy_(information_theory)}{Shannon entropy} of their model weights \(\tilde w_i\):
\begin{equation}
A = 
...
\hline
Effective Sample Size \$\textbackslash{}left(\textbackslash{}widehat\{\textbackslash{}text\{ESS\}\}\textbackslash{}right)\$ & \$\textbackslash{}gt 500\$ & [Gelman et al. 2014](https://sites.stat.columbia.edu/gelman/book/)\\
\hline
...
\section*{November 25, 2024 --- The MOSAIC framework presented at ASTMH 2024}\label{november-25-2024-the-mosaic-framework-presented-at-astmh-2024}
\addcontentsline{toc}{section}{November 25, 2024 --- The MOSAIC framework presented at ASTMH 2024}
