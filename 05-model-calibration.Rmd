...
metric_tbl <- data.frame(
  Metric            = c("Effective Sample Size $\\left(\\widehat{\\text{ESS}}\\right)$",
                        "",
                        "Agreement Index $\\left(A\\right)$",
                        "Weight Coefficient of Variation $\\left(\\mathrm{CV}_{\\tilde{\\mathbf w}} \\right)$"),
  
  `Target range`     = c("$\\gt 500$",
                         "$\\gt 1000$",
                        "$\\gt 0.7 \\ \\text{or} \\ 0.8$",
                        "$\\lt 1 \\ \\text{or} \\ 2$"),
  
  #`Action if missed` = c("Increase $n_{\\text{sim}}$, relax $\Delta \text{AIC}$ cut-off, reduce ESS threshold to 500",
  #                      "Increase $n_{\\text{sim}}$, relax $\Delta \text{AIC}$ cut-off, more informative priors, reduce $A$ threshold to 0.7",
  #                      "Increase $n_{\\text{sim}}$, reduce number of fitted parameters, increase $\\mathrm{CV}_w$ to 3, severe skew suggests instability"),
  
  Source             = c("[Gelamn et al 2014](https://sites.stat.columbia.edu/gelman/book/)",
                         "[Bürkner 2017](https://www.jstatsoft.org/article/view/v080i01)",
                         "[Elvira et al 2022](https://onlinelibrary.wiley.com/doi/10.1111/insr.12500)",
                         "[Kong et al 1994](https://doi.org/10.1080/01621459.1994.10476469)")
)
...
kable(metric_tbl,
      ...
      col.names = c('Metric', 
                    'Target range', 
                    #'Action if target missed', 
                    'Source'),
      caption  = "Detials on convergence diagnostics with recommended thresholds and troubleshooting guidelines.")

```

```{r likelihood-example, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', out.width='95%', fig.cap='Example log-Likelihood curve for a sample of 2000 model simulations. Log-likelihood values are sorted from minumum to maximum with the model simulation giving the maximum likelihood highlighted in green.'}
knitr::include_graphics("figures/likelihood_example.png")
```
